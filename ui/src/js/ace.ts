...
export const Ace2Editor = function (this: any) {
  ...
  this.init = async function (containerId: string, initialCode: string) {
    ...
    // Create and initialize the AceEditor web component.
    editor = new AceEditor(editorBody);
    await editor.init();

    // Accessibility (upstream #7778 / #7255 / #7782 / #7758): expose the
    // contenteditable as a browsable document rather than a textbox — a
    // textbox role forces NVDA/JAWS into focus mode, flattening the pad into
    // one edit field and hiding links/headings from the rotor. A label plus a
    // screen-reader-only keyboard hint describe how to enter/leave the editor.
    // The hint lives on the editor container (resolved by id via
    // aria-describedby) so the AceEditor line model never wipes it.
    editorBody.setAttribute('aria-label', 'Pad content');
    editorBody.setAttribute('aria-describedby', 'editor-keyboard-hint');
    if (!document.getElementById('editor-keyboard-hint')) {
